...
*   **🌊 【修复】ATRI 报告静态资源完全本地化**：
    *   **远程资源随插件打包**：将 ATRI 模板引用的 3 个字体、17 个 WebP 与 13 个 GIF 全部纳入插件，消除对外部素材镜像的运行时依赖。
    *   **远程 T2I 兼容内联**：渲染前自动将本地资源转换为 Data URI；即使使用 AstrBot 公共 T2I 服务，截图端也无需再次联网下载字体和装饰图。
    *   **截图负载精简**：图片报告仅加载一套正文中文字体，并优先使用本地静态 WebP 代替 GIF，将 ATRI 截图 HTML 体积降低约 67%；独立 HTML 报告仍保留完整字体和动画。
